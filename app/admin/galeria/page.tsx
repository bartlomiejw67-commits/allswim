        ...
        {images?.map((img) => (
          <div key={img._id} style={{ ...card, padding: 12 }}>
            {img.url && (img.kind === "video" ? (
              <video src={img.url} muted playsInline controls style={{ width: "100%", aspectRatio: "4/3", objectFit: "cover", borderRadius: 12, display: "block", background: "#000" }} />
            ) : (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={img.url} alt={img.caption ?? ""} style={{ width: "100%", aspectRatio: "4/3", objectFit: "cover", borderRadius: 12, display: "block" }} />
            ))}
